...
type IExports =
    ...
    abstract equal:
        actual: obj * expected: obj option * ?message: U2<string, Error> -> unit
    ...
    abstract notDeepEqual:
        actual: obj * expected: obj option * ?message: U2<string, Error> -> unit

    abstract strictEqual:
        actual: obj * expected: 'T * ?message: U2<string, Error> -> unit

    abstract notStrictEqual:
        actual: obj * expected: 'T * ?message: U2<string, Error> -> unit

    abstract deepStrictEqual:
        actual: obj * expected: 'T * ?message: U2<string, Error> -> unit
    ...
    abstract throws:
        block: (unit -> obj option) *
        error: AssertPredicate *
        ?message: U2<string, Error> ->
            unit

    abstract doesNotThrow:
        block: (unit -> obj option) * ?message: U2<string, Error> -> unit
    ...
    abstract ``match``:
        value: string * regExp: RegExp * ?message: U2<string, Error> -> unit

    abstract doesNotMatch:
        value: string * regExp: RegExp * ?message: U2<string, Error> -> unit

    abstract strict: obj

[<AllowNullLiteral; AbstractClass; Erase>]
type AssertionError =
    inherit Error
